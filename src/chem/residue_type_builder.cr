# TODO: move structure definition into another class so that it's
# yieled inside the `structure` block.
class Chem::ResidueType::Builder
  ...
  def build : ResidueType
    fail "Missing residue description" unless (description = @description)
    fail "Missing residue name" if @names.empty?
    fail "Missing residue atom names" if @atom_types.empty?
  ...
  def sidechain
    fail "Sidechain is only valid for protein residues" unless @kind.protein?
    with self yield
  end
  ...
  private def atom_type!(name : String) : AtomType
    atom_type?(name) || fail "Unknown atom type #{name}"
  end
  ...
  private def add_bond(atom_t : AtomType, other : AtomType, order : Int = 1)
    ...
    if bond && bond.order != order
      fail "Bond #{atom_t.name}#{bond.to_char}#{other.name} already exists"
    elsif bond.nil?
  ...
  private def check_valencies!
    @atom_types.each do |atom_t|
      ...
      next if num == 0
      fail "Atom type #{atom_t} has incorrect valency (#{atom_t.valency - num}), " \
