module Chem
  ...
  macro finished
    ...
    {% annotations = klasses.map &.annotation(RegisterFormat) %}
    ...
    {% for format in file_formats %}
      {% for ary in [readers, writers] %}
        {% ary = ary.select &.annotation(RegisterFormat)[:format].id.==(format) %}
        {% if ary.size > 1 %}
    ...
    {% for klass in klasses %}
      {% format = klass.annotation(RegisterFormat)[:format].id %}
      {% if extnames = klass.annotation(RegisterFormat)[:ext] %}
        {% for ext in extnames %}
      ...
      {% if names = klass.annotation(RegisterFormat)[:names] %}
        {% for name in names %}
