...
describe Chem::Spatial::Bounds do
  ...
  describe "#+" do
    it "translates a bounds by a vector" do
      (Bounds[1, 2, 3] + Vec3[3, 2, 1]).should eq Bounds.new(Vec3[3, 2, 1], S[1, 2, 3])

      bounds = Bounds.new Vec3[6, 1, 0], S[6, 6.5, 7]
      (bounds + Vec3[1.5, 7.3, 1]).should eq Bounds.new(Vec3[7.5, 8.3, 1], S[6, 6.5, 7])
    end
  end
  ...
  describe "#-" do
    it "translates a bounds by a vector" do
      ...
      bounds = Bounds.new Vec3[6, 1, 0], S[6, 6.5, 7]
      (bounds - Vec3[1.5, 7.3, 1]).should eq Bounds.new(Vec3[4.5, -6.3, -1], S[6, 6.5, 7])
    end
  end
  ...
  #     bounds = Bounds.new Vec3[6, 1, 0], S[1.3, 6.5, 8.2]
  #     (bounds * 2.5).should eq Bounds.new(Vec3[6, 1, 0], S[3.25, 16.25, 20.5])
  #   end
  # end
  ...
  #     bounds = Bounds.new Vec3[6, 1, 0], S[1.5, 5, 10]
  #     (bounds / 0.25).should eq Bounds.new(Vec3[6, 1, 0], S[6, 20, 40])
  #   end
  # end
  ...
  describe "#center" do
    it "returns the center of the bounds" do
      ...
      Bounds.new(Vec3[1, 2, 3], S[6, 3, 23]).center.should eq Vec3[4, 3.5, 14.5]
    end
  end
  ...
  describe "#includes?" do
    it "tells if a vector is within the primary unit cell" do
      ...
      Bounds.new(Vec3[1, 2, 3], S[6, 3, 23]).includes?(Vec3[3, 2.1, 20]).should be_true
      Bounds[10, 20, 30].includes?(Vec3[-1, 2, 3]).should be_false
      Bounds.new(Vec3[1, 2, 3], S[6, 3, 23]).includes?(Vec3[2.4, 1.8, 23.1]).should be_false
    end
    ...
    context "given a bounds" do
      it "returns true when enclosed" do
        ...
        bounds.includes?(Bounds.new(Vec3[1, 2, 3], S[5, 4, 6])).should be_true
        bounds.includes?(Bounds.new(S[5, 4, 6])).should be_true
      end

      it "returns false when intersected" do
        bounds = Bounds.new S[10, 10, 10], 90, 90, 120
        bounds.includes?(Bounds.new(Vec3[-1, 2, -4.3], S[5, 4, 6])).should be_false
      end
  ...
  describe "#volume" do
    it "returns the volume enclosed by the bounds" do
      ...
      Bounds.new(Vec3[1, 2, 3], S[6, 3, 23]).volume.should eq 414
    end
  end
end
