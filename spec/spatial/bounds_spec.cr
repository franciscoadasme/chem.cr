...
describe Chem::Spatial::Bounds do
  ...
  describe "#+" do
    it "translates a bounds by a vector" do
      (Bounds[1, 2, 3] + Vec3[3, 2, 1]).should eq Bounds.new(Vec3[3, 2, 1], Size[1, 2, 3])

      bounds = Bounds.new Vec3[6, 1, 0], Size[6, 6.5, 7]
      (bounds + Vec3[1.5, 7.3, 1]).should eq Bounds.new(Vec3[7.5, 8.3, 1], Size[6, 6.5, 7])
    end
  end
  ...
  describe "#-" do
    it "translates a bounds by a vector" do
      ...
      bounds = Bounds.new Vec3[6, 1, 0], Size[6, 6.5, 7]
      (bounds - Vec3[1.5, 7.3, 1]).should eq Bounds.new(Vec3[4.5, -6.3, -1], Size[6, 6.5, 7])
    end
  end
  ...
  #     bounds = Bounds.new Vec3[6, 1, 0], Size[1.3, 6.5, 8.2]
  #     (bounds * 2.5).should eq Bounds.new(Vec3[6, 1, 0], Size[3.25, 16.25, 20.5])
  #   end
  # end
  ...
  #     bounds = Bounds.new Vec3[6, 1, 0], Size[1.5, 5, 10]
  #     (bounds / 0.25).should eq Bounds.new(Vec3[6, 1, 0], Size[6, 20, 40])
  #   end
  # end
  ...
  describe "#center" do
    it "returns the center of the bounds" do
      ...
      Bounds.new(Vec3[1, 2, 3], Size[6, 3, 23]).center.should eq Vec3[4, 3.5, 14.5]
    end
  end

  describe "#each_vertex" do
    it "yields bounds' vertices" do
      bounds = Bounds.new Size[8.77, 9.5, 24.74], 88.22, 80, 70.34
  ...
  describe "#includes?" do
    it "tells if a vector is within the primary unit cell" do
      ...
      Bounds.new(Vec3[1, 2, 3], Size[6, 3, 23]).includes?(Vec3[3, 2.1, 20]).should be_true
      Bounds[10, 20, 30].includes?(Vec3[-1, 2, 3]).should be_false
      Bounds.new(Vec3[1, 2, 3], Size[6, 3, 23]).includes?(Vec3[2.4, 1.8, 23.1]).should be_false
    end

    it "tells if a vector is within the primary unit cell (non-orthogonal)" do
      lattice = Bounds.new Size[23.803, 23.828, 5.387], 90, 90, 120
    ...
    context "given a bounds" do
      it "returns true when enclosed" do
        bounds = Bounds.new Size[10, 10, 10], 90, 90, 120
        bounds.includes?(Bounds.new(Vec3[1, 2, 3], Size[5, 4, 6])).should be_true
        bounds.includes?(Bounds.new(Size[5, 4, 6])).should be_true
      end

      it "returns false when intersected" do
        bounds = Bounds.new Size[10, 10, 10], 90, 90, 120
        bounds.includes?(Bounds.new(Vec3[-1, 2, -4.3], Size[5, 4, 6])).should be_false
      end
  ...
  describe "#max" do
    ...
    context "given a non-orthogonal bounds" do
      it "returns the maximum edge" do
        bounds = Bounds.new(Vec3[1.5, 3, -0.4], Size[10, 10, 12], 90, 90, 120)
  ...
  describe "#translate" do
    it "translates the origin" do
      bounds = Bounds.new Vec3[-5, 1, 20], Size[10, 10, 10], 90, 90, 120
      bounds = bounds.translate(Vec3[1, 2, 10])
      bounds.min.should eq Vec3[-4, 3, 30]
      bounds.size.should eq Size[10, 10, 10]
  ...
  describe "#pad" do
    context "given an orthogonal bounds" do
      it "returns a padded bounds" do
        ...
        padded = bounds.pad(2)
        padded.size.should eq Size[14, 14, 14]
    ...
    context "given a non-orthogonal bounds" do
      it "returns a padded bounds" do
        bounds = Bounds.new(Size[4, 7, 8.5], 90, 120, 90)
        padded = bounds.pad(0.5)
        padded.size.should eq Size[5, 8, 9.5]
  ...
  describe "#vertices" do
    it "returns bounds' vertices" do
      vertices = Bounds.new(Size[10, 10, 10], 90, 90, 120).vertices
      vertices.size.should eq 8
      vertices.should be_close [
        Vec3[0, 0, 0],
        Vec3[0, 0, 10],
  ...
  describe "#volume" do
    it "returns the volume enclosed by the bounds" do
      ...
      Bounds.new(Size[5, 5, 8], 90, 90, 120).volume.should be_close 173.2050807569, 1e-10
      Bounds.new(Size[1, 2, 3], beta: 101.2).volume.should be_close 5.8857309321, 1e-10
      Bounds.new(Vec3[1, 2, 3], Size[6, 3, 23]).volume.should eq 414
    end
  end
end
