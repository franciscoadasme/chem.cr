...
describe Chem::Spatial::Transform do
  describe ".aligning" do
    it "returns a transformation that aligns two vectors" do
      u = vec3(1, 2, 4)
      v = vec3(-1, 1, 25)
      transform = Chem::Spatial::Transform.aligning(u, v)
      (transform * u).normalize.should eq v.normalize
    end

    it "returns a transformation that aligns two pairs of vectors" do
      transform = Chem::Spatial::Transform.rotation(vec3(1, 2, 3), 65)
      u = vec3(-5, 1, 3)
      v = vec3(6, 2, 62)
      uu = transform * u
      vv = transform * v
      transform = Chem::Spatial::Transform.aligning({uu, vv}, to: {u, v})
    ...
    it "returns the identity matrix for the same coordinate set" do
      s = Array(Chem::Structure).read spec_file("E20_conformers.mol2")
      transform = Chem::Spatial::Transform.aligning(s[0], s[0])
      transform.should eq Chem::Spatial::Transform.identity
    end
  end

  describe ".identity" do
    it "creates an identity transformation" do
      transform = Chem::Spatial::Transform.identity
  ...
  describe ".rotation" do
    it "returns a rotation transformation" do
      transform = Chem::Spatial::Transform.rotation(vec3(1, 0, 0), 90)
      ...
      transform = Chem::Spatial::Transform.rotation(vec3(0.67, 0.68, 0.3), 180)
      transform.linear_map.should be_close Chem::Spatial::Mat3[
      ...
      transform = Chem::Spatial::Transform.rotation(vec3(0.67, 0.68, 0.3), 360)
      transform.linear_map.should be_close Chem::Spatial::Mat3.identity, 1e-15
      transform.offset.should eq [0, 0, 0]

      transform = Chem::Spatial::Transform.rotation(vec3(0.31, 0.91, -0.28), 46)
      transform.linear_map.should be_close Chem::Spatial::Mat3[
    ...
    it "returns the rotation by Euler angles" do
      transform = Chem::Spatial::Transform.rotation(123, 4, 96)
      transform.linear_map.should be_close Chem::Spatial::Mat3[
  ...
  describe ".scaling" do
    it "returns a scaling transformation" do
      transform = Chem::Spatial::Transform.scaling(1.5)
    ...
    it "returns a scaling transformation with per-axis factors" do
      transform = Chem::Spatial::Transform.scaling(1, 3, 4)
  ...
  describe ".translation" do
    it "returns a translation transformation" do
      transform = Chem::Spatial::Transform.translation(vec3(1, 2, 3))
      transform.linear_map.should eq Chem::Spatial::Mat3.identity
  ...
  describe "#*" do
    it "transforms a vector" do
      (Chem::Spatial::Transform.translation(vec3(1, -2, 3)) * vec3(0, 0, 0)).should eq [1, -2, 3]
  ...
  describe "#inv" do
    it "returns the inverse of the transformation" do
      transform = Chem::Spatial::Transform.scaling(2.41)
      (transform.inv * transform).should eq Chem::Spatial::Transform.identity
      (transform * transform.inv).should eq Chem::Spatial::Transform.identity
    end
  end
  ...
  describe "#rotate" do
    it "rotates the transformation" do
      ...
      angle = rand * 180
      transform = Chem::Spatial::Transform.identity.translate(vec3(1, 2, 3)).scale(2.5)
      expected = Chem::Spatial::Transform.rotation(rotaxis, angle) * transform
  ...
  describe "#rotation" do
    it "returns the rotation component" do
      transform = Chem::Spatial::Transform.rotation(50, 10, 5).translate(vec3(1, 2, 3))
      rot = transform.rotation
      rot.should be_a Chem::Spatial::Transform
  ...
  describe "#scale" do
    it "scales the transformation" do
      ...
      transform = Chem::Spatial::Transform.translation offset
  ...
  describe "#to_s" do
    it "returns a string representation" do
      expected = "[\
        ...
        [0  0  0  1]]"
      transform = Chem::Spatial::Transform.translation(vec3(1, 2, 3)).scale(2).rotate(45, 15, 90)
  ...
  describe "#translate" do
    it "translates the transformation" do
      offset = vec3(1, 2, 3)
      transform = Chem::Spatial::Transform.identity.translate(offset)
      transform.linear_map.should eq Chem::Spatial::Mat3.identity
  ...
  describe "#translation" do
    it "returns the translation component" do
      ...
      trans.should be_a Chem::Spatial::Transform
