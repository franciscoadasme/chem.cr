...
describe Chem::Spatial::Vec3 do
  ...
  describe ".[]" do
    it "returns a vector with the given components" do
      Vec3[1, 2, 3].should eq Vec3.new(1, 2, 3)
    end
  end
  ...
  describe "#+" do
    ...
    it "sums a vector and a size" do
      (Vec3[1, 2, 3] + Size3[3, 2, 1]).should eq Vec3[4, 4, 4]
    end
  end

  describe "#-" do
    it "returns the inverse vector" do
      (-v1).should eq Vec3[-3, -4, 0]
    end

    it "returns the element-wise subtraction with a number" do
      (Vec3[3, 9, 123] - 5).should eq Vec3[-2, 4, 118]
    end
    ...
    it "subtracts a size from a vector" do
      (Vec3[1, 2, 3] - Size3[3, 2, 1]).should eq Vec3[-2, 0, 2]
    end
  end

  describe "#*" do
    it "returns the multiplication with a number" do
      (v1 * 3).should eq Vec3[9, 12, 0]
    end
  ...
  describe "#cross" do
    it "returns the cross product between two vectors" do
      Vec3[1, 0, 0].cross(Vec3[0, 1, 0]).should eq Vec3[0, 0, 1]
      Vec3[2, 3, 4].cross(Vec3[5, 6, 7]).should eq Vec3[-3, 6, -3]
    end
  end
  ...
  describe "#image" do
    it "returns vector's pbc image" do
      ...
      vec.image(1, 0, 0).should eq Vec3[1.456, 0.1, 0.8]
      vec.image(-1, 0, 0).should eq Vec3[-0.544, 0.1, 0.8]
      vec.image(-1, 1, -5).should eq Vec3[-0.544, 1.1, -4.2]
    end
  end
  ...
  describe "#image" do
    it "returns vector's pbc image" do
      ...
      vec.image(lat, -1, 1, -5).should be_close Vec3[-18.308592, 18.870709, -120.433621], 1e-6
    end
  end

  describe "#inv" do
    it "returns the inverse vector" do
      v1.inv.should eq Vec3[-3, -4, 0]
    end
  end
  ...
  describe "#map_with_index" do
    it "returns the element-wise map operation of the vector" do
      Vec3[2, 75, 2].map_with_index { |value, i| value + i**2 }.should eq Vec3[2, 76, 6]
    end
  end
  ...
  describe "#proj" do
    it "returns the projection on the vector" do
      Vec3[1, 2, 3].proj(Vec3[1, 0, 1]).should be_close Vec3[2, 0, 2], 1e-15
    end
  end

  describe "#proj_plane" do
    it "returns the projection on the plane" do
      Vec3[1, 2, 3].proj_plane(Vec3[1, 0, 1]).should be_close Vec3[-1, 2, 1], 1e-15
    end
  end

  describe "#resize" do
    it "resizes a vector to an arbitrary size" do
      Vec3[1, 0, 0].resize(5).should eq Vec3[5, 0, 0]
      (Vec3[1, 1, 0] * 4.2).resize(1).should eq Vec3[Math.sqrt(0.5), Math.sqrt(0.5), 0]
    end
  end
  ...
  describe "#wrap" do
    it "wraps a fractional vector" do
      Vec3[0, 0, 0].wrap.should eq Vec3[0, 0, 0]
      Vec3[1, 1, 1].wrap.should eq Vec3[1, 1, 1]
      Vec3[0.5, 0.3, 1].wrap.should eq Vec3[0.5, 0.3, 1]
      Vec3[0.5, 0.3, 1.1].wrap.should be_close Vec3[0.5, 0.3, 0.1], 1e-15
      Vec3[0.01, -0.3, 2.4].wrap.should be_close Vec3[0.01, 0.7, 0.4], 1e-15
    end
    ...
    it "wraps a fractional vector around a center" do
      ...
      Vec3[0.5, 0.3, 1].wrap(center).should eq Vec3[0.5, 0.3, 0]
      Vec3[0.5, 0.3, 1.1].wrap(center).should be_close Vec3[0.5, 0.3, 0.1], 1e-15
      Vec3[0.01, -0.3, 2.4].wrap(center).should be_close Vec3[0.01, -0.3, 0.4], 1e-12
    end
