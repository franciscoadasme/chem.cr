...
describe Chem::Spatial::CoordinatesProxy do
  ...
  describe "#align_to" do
    it "aligns coordinates to a reference" do
      ...
      ref_pos = s[0].coords
      s[1].coords.align_to(ref_pos).rmsd(ref_pos).should be_close 3.463298, 1e-6
      s[2].coords.align_to(ref_pos).rmsd(ref_pos).should be_close 1.818679, 1e-6
      s[3].coords.align_to(ref_pos).rmsd(ref_pos).should be_close 3.845655, 1e-6
      s[4].coords.align_to(ref_pos).rmsd(ref_pos).should be_close 1.475276, 1e-6
    end
  end
  ...
  describe ".rmsd" do
    it "computes the rmsd in-place" do
      s = Array(Chem::Structure).read spec_file("E20_conformers.mol2")
      s[1].coords.rmsd(s[0].coords).should be_close 7.933736, 1e-6
      s[2].coords.rmsd(s[0].coords).should be_close 2.607424, 1e-6
      s[3].coords.rmsd(s[0].coords).should be_close 8.177316, 1e-6
      s[4].coords.rmsd(s[0].coords).should be_close 1.815176, 1e-6
    end

    it "computes the rmsd in-place with weights" do
      s = Array(Chem::Structure).read spec_file("E20_conformers.mol2")
      weights = s[0].atoms.map &.mass
      s[1].coords.rmsd(s[0].coords, weights).should be_close 8.187659, 1e-6
      s[2].coords.rmsd(s[0].coords, weights).should be_close 2.265467, 1e-6
      s[3].coords.rmsd(s[0].coords, weights).should be_close 7.955341, 1e-6
      s[4].coords.rmsd(s[0].coords, weights).should be_close 1.510461, 1e-6
    end

    it "computes the minimum rmsd" do
      s = Array(Chem::Structure).read spec_file("E20_conformers.mol2")
      s[1].coords.rmsd(s[0].coords, minimize: true).should be_close 3.463298, 1e-6
      s[2].coords.rmsd(s[0].coords, minimize: true).should be_close 1.818679, 1e-6
      s[3].coords.rmsd(s[0].coords, minimize: true).should be_close 3.845655, 1e-6
      s[4].coords.rmsd(s[0].coords, minimize: true).should be_close 1.475276, 1e-6
    end

    it "computes the minimum rmsd with weights" do
      s = Array(Chem::Structure).read spec_file("E20_conformers.mol2")
      weights = s[0].atoms.map &.mass
      s[1].coords.rmsd(s[0].coords, weights, minimize: true).should be_close 2.811033, 1e-6
      s[2].coords.rmsd(s[0].coords, weights, minimize: true).should be_close 1.358219, 1e-6
      s[3].coords.rmsd(s[0].coords, weights, minimize: true).should be_close 3.067433, 1e-6
      s[4].coords.rmsd(s[0].coords, weights, minimize: true).should be_close 1.084173, 1e-6
    end
  end
  ...
  describe "#transform" do
    ...
    it "aligns by a subset (#183)" do
      ...
      atoms.coords.rmsd(ref_atoms.coords).should be_close 0.091, 1e-3
    end
