...
describe Chem::VASP::Chgcar do
  describe ".read" do
    it "reads a CHGCAR" do
      path = spec_file("vasp/CHGCAR")
      grid = Chem::VASP::Chgcar.read path
    ...
    it "reads a CHGCAR header" do
      path = spec_file("vasp/CHGCAR")
      info = Chem::VASP::Chgcar.read_info path
      info.bounds.should eq bounds(10, 10, 10)
      info.dim.should eq({2, 2, 2})
    end

    it "reads the attached structure" do
      path = spec_file("vasp/CHGCAR")
      structure = Chem::VASP::Chgcar.read_structure path
  ...
  describe ".write" do
    it "writes a CHGCAR" do
      ...
      io = IO::Memory.new
      Chem::VASP::Chgcar.write io, grid, structure
    ...
    it "fails when writing a CHGCAR with a non-periodic structure" do
      ...
      expect_raises Chem::Spatial::NotPeriodicError do
        Chem::VASP::Chgcar.write io, grid, Chem::Structure.new
      end
    end
    ...
    it "fails when cell and bounds are incompatible" do
      ...
      expect_raises ArgumentError, "Incompatible structure and grid" do
        Chem::VASP::Chgcar.write io, make_grid({3, 3, 3}, {20, 20, 20}), structure
      end
    end
