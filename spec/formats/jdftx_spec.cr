...
describe Chem::JDFTx do
  describe ".read" do
    it "parses a JDFTx file" do
      path = spec_file("polyala-val.ionpos")
      structure = Chem::JDFTx.read(path, guess_bonds: true, guess_names: true)
    ...
    it "parses a split JDFTx file" do
      ...
      structure = Chem::JDFTx.read(path)
  ...
  describe ".write" do
    it "writes a JDFTx file" do
      path = spec_file("polyala-val.ionpos")
      struc = Chem::JDFTx.read path
      expected = reduced_cell_prec File.read(path)

      io = IO::Memory.new
      Chem::JDFTx.write io, struc
      reduced_cell_prec(io.to_s).should eq expected
    end
    ...
    it "writes a split JDFTx file" do
      ...
      tmp = File.tempfile(".ionpos") do |io|
        Chem::JDFTx.write io, struc, single_file: false
      end
    ...
    it "writes fractional coordinates" do
      ...
      io = IO::Memory.new
      Chem::JDFTx.write io, struc, fractional: true
      reduced_cell_prec(io.to_s).should eq reduced_cell_prec(expected)
    end
  end
end
